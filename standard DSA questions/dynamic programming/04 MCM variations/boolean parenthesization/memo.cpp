...
int solve(string &s, int i, int j, bool isTrue, vector<vector<vector<int>>> &t){
    ...
    for(int k = i+1; k < j; k+=2){
        ll leftTrue = solve(s, i, k-1, 1, t);
        ll leftFalse = solve(s, i, k-1, 0, t);
        ll rightTrue = solve(s, k+1, j, 1, t);
        ll rightFalse = solve(s, k+1, j, 0, t);
...
int evaluateExp(string & exp) {
    //i, j, isTrue changing :: think about their ranges
    int n = exp.size();
    vector<vector<vector<int>>> t(n, vector<vector<int>>(n, vector<int>(2, -1)));

    return solve(exp, 0, exp.size()-1, true, t);
}